...
const DAILY_QUESTIONS = [
  ...
  // 💗 Συναισθηματικές
  "Τι ένιωσες για μένα σήμερα;",
  "Ποιο μικρό πράγμα έκανα και σου άρεσε;",
  "Πότε ένιωσες πιο κοντά μου σήμερα;",
  "Τι σε έκανε να νιώσεις ασφάλεια σήμερα;",
  "Ποια σκέψη για εμάς σε ηρέμησε σήμερα;",
  "Αν μου έστελνες ένα emoji σήμερα, ποιο θα ήταν και γιατί;",

  // 🔥 Ρομαντικές
  "Τι είναι κάτι που θες να κάνουμε μαζί σύντομα;",
  "Τι θα ήθελες να σου πω πριν κοιμηθούμε;",
  "Ποια στιγμή μας σκέφτηκες πιο έντονα σήμερα;",
  "Τι αγαπάς περισσότερο σε εμάς;",
  "Ποιο είναι ένα πράγμα που σε κάνει να με διαλέγεις κάθε μέρα;",

  // 🌙 Βραδινές / Reflection
  "Ποια ήταν η καλύτερη στιγμή της μέρας σου;",
  "Τι σε κούρασε σήμερα;",
  "Τι θα ήθελες να αλλάξει αύριο;",
  "Ποιο πράγμα σε έκανε να πεις «άξιζε η μέρα»;",
  "Αν η μέρα τελείωνε με μια αγκαλιά, πώς θα ήταν;",

  // 🧠 Fun / Δημιουργικές
  "Αν ήμασταν σκηνή σε ταινία σήμερα, τι θα συνέβαινε;",
  "Αν η μέρα μας είχε τίτλο, ποιος θα ήταν;",
  "Αν μπορούσαμε να τηλεμεταφερθούμε τώρα, πού θα ήθελες να είμαστε;",
  "Αν έπρεπε να με περιγράψεις με μία λέξη σήμερα, ποια θα ήταν;",
  "Ποια ανάμνησή μας σου ήρθε ξαφνικά σήμερα;",

  // 🫂 Επικοινωνία
  "Τι θα ήθελες να καταλάβω καλύτερα για σένα σήμερα;",
  "Υπάρχει κάτι που κράτησες μέσα σου σήμερα;",
  "Τι χρειάζεσαι περισσότερο από μένα αυτή την περίοδο;",
  "Τι μπορώ να κάνω για να σε στηρίξω καλύτερα;",
  "Τι ένιωσες όταν μιλήσαμε σήμερα;",

  // 💞 Μαζί
  "Ποιο είναι κάτι μικρό που κάνουμε και σε κάνει χαρούμενη/ο;",
  "Ποια συνήθεια μας αγαπάς περισσότερο;",
  "Ποια στιγμή μας σε κάνει να χαμογελάς πάντα;",
  "Τι είναι αυτό που κάνει εμάς… εμάς;",
  "Ποιο «εμείς» θες να κρατήσουμε για πάντα;"
];
...
async function saveDailyAnswer(whoKey) {
  ...
  try {
    ...
    await runTransaction(db, async (tx) => {
      ...
      const sData = streakSnap.exists() ? streakSnap.data() : {};

      // αν είναι ήδη locked, δεν επιτρέπουμε αλλαγή
      const alreadyLocked = !!qaData?.answers?.[whoKey]?.locked;
      if (alreadyLocked) return;

      const otherKey = whoKey === "giorgos" ? "venia" : "giorgos";
      const otherLocked = !!qaData?.answers?.[otherKey]?.locked;

      const nowComplete = otherLocked; // αφού εμείς θα κλειδώσουμε τώρα, complete αν ο άλλος είναι ήδη locked
      const wasComplete = !!qaData?.complete;

      // ✅ ΜΕΤΑ ΤΑ READS: WRITES
      tx.set(
        qaRef,
        {
          dayKey,
      ...
      // μόλις γίνει complete για πρώτη φορά σήμερα
